"""
Test NON-PINS account and return full API response
"""
...
def test_non_pins_account():
    """Test NON-PINS account and show all API response fields"""
    
    config = Config()
    session_token = config.BREEZE_SESSION_TOKEN_NON_PINS
    api_key = config.BREEZE_API_KEY
    
    print("\n" + "="*80)
    print("BREEZE API RESPONSE FOR NON-PINS ACCOUNT")
    print("="*80)
    print(f"Session Token: {session_token}")
    print(f"API Key: {api_key[:20]}...")
    
    if session_token.startswith('<your-'):
        print("\n⚠️  ERROR: NON-PINS session token not configured")
        print("Update .env with your NON-PINS session token:")
        print("  BREEZE_SESSION_TOKEN_NON_PINS=<your-actual-token>")
        return None
